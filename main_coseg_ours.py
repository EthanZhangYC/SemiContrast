...
#             logger.info("checkpoint state dict:", state_dict.keys())
#             logger.info("model state dict:", model.state_dict().keys())
#             logger.info("loaded pretrained model:", opt.pretrained_model_path)
...
#         # logger.info info
#         if (idx + 1) % opt.logger.info_freq == 0:
...
def train(train_loader, model, cls_head, criterion, logger_tb, optimizer, epoch, opt, dice_loss, ce_loss, logger):
    ...
    for idx in range(num_batches):
        ...
        images_unl = torch.cat([images_unl_source[0],images_unl_target[0],images_unl_source[1],images_unl_target[1]], dim=0)
        images_unl = images_unl.cuda()
            
        bsz = images_unl.shape[0]//2
        feat_unl,_,_,_ = model(images_unl)
        feat_unl = F.normalize(feat_unl, dim=1)
        # feat_unl = feat_unl[:feat_unl.shape[0]//2]
        # out_unl = cls_head(feat_unl.permute(0,2,3,1).reshape(-1,64))
        # out_unl = out_unl.view(-1,256,256,5).permute(0,3,1,2)
        f1, f2 = torch.split(feat_unl, [bsz, bsz], dim=0)
        feat_unl = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)  # [bsz, n_view, c, img_size, img_size]
        
        loss_con = criterion(feat_unl) 
        if loss_con.mean() == 0:
            continue
        mask_con = (loss_con != 0)
        mask_con = mask_con.int().cuda()
